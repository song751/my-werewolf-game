/* ========================================
   双身份狼人杀 - V10.1 (UI / UX 优化版)
   - 整合狼人操作面板至主界面，移除独立模态框
   - 更新玩家卡片UI，使用角标展示狼人投票
   - 优化身份配置、加入游戏等流程的交互
   - 将所有样式移至CSS文件，实现代码分离
======================================== */
...
const App = {
  ...
  handleGlobalClick(e) {
    ...
    if (action === 'open-wolf') { // This action is now obsolete
      return;
    }
    ...
    // The original full switch logic from V10.0 is used here, with 'open-wolf' removed.
    switch (action) {
  ...
  // This function is obsolete as the wolf modal is removed.
  openWolfModal() {},
  ...
  renderActionPanel() {
    ...
    if (this.gameState.phase === 'SHERIFF_SPEECH') {
      ...
      if (isC && !drop) {
        panel.innerHTML = `<div class="drop-water"><button class="action-btn" data-action="sheriff-drop">💧 退水</button></div>`;
            ...
            if (ns.wolf === 'pending') {
                ...
                panel.innerHTML = `
                    <div class="wolf-inline-panel">
                        <div class="wolf-hint">${can ? '🎯 点击上方玩家卡片即可投票' : '⏳ 等待普通狼人行动'}</div>
                        <div id="wolf-votes-display" class="wolf-votes-section"></div>
                        ${role === '狼人' ? `
                            <div id="wolf-chat-area" class="wolf-chat-section">
